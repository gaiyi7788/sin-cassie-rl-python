    ...
    class TensorboardCallback(BaseCallback):
        ...
        def __init__(self, log_dir, verbose=0):
            ...
            if cfg['trainer']['resume']:
                with open(os.path.join(self.log_dir, "resume.txt"),"w") as f:
                    f.write("resume from: \n")
                    f.write(cfg['trainer']['resume_path'])
                self.n_calls = int(cfg['trainer']['resume_path'].split('/')[-1].split('_')[-1].replace(".zip",""))
                print("resume_calls: ", self.n_calls)
